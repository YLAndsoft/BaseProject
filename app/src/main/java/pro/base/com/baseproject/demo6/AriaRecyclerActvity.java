...
public class AriaRecyclerActvity extends BaseActivity implements DownloadOnClickListener {
    @ViewInject(value = R.id.aria_nestedScrollview)
    NestedScrollView mScrollview;
    @ViewInject(value = R.id.aria_top_image)
    ImageView aria_top_image;
    @ViewInject(value = R.id.aria_recyclerView)
    RecyclerView mRecyclerView;

    private AriaRecylcerAdapter mAdapter;
    ...
    public int bindLayout() {
        return R.layout.aria_recycler_activity;
    }
    @Override
                ...
                if(result!=null){
                    ...
                    if(newGameAppTypeViews!=null&&newGameAppTypeViews.size()>0){
                        if(viewType==0){
                            ...
                            llManager = new GridLayoutManager(mContext, 3);
                        }
                        ...
                        mAdapter = new AriaRecylcerAdapter(mContext,viewType,AriaRecyclerActvity.this);
                        //设置布局管理器
            ...
            switch (taskState){
                ...
                case IEntity.STATE_COMPLETE://完成状态
                    ...
                    if(AppManageUtils.validateSdcardHasAPK(app)){  //文件是否下载完成，并且完整
                        //安装
                        String s = AppManageUtils.installApp(app.getAppApkUrl(), AriaRecyclerActvity.this);
                        if(s!=null&&!s.equals("")){
                            showToast(s);
                        }
                    }else{
    ...
    @Download.onTaskStart void taskStart(DownloadTask task){
        ...
        if(download!=null){
            if(mAdapter!=null){
                mAdapter.refreshItem(download,task,llManager,mRecyclerView);
            }
        }
    }
    /**
